<style scoped>
	.datetime-picker {
		position: relative;
		display: inline-block;
		font-family: "Segoe UI", "Lucida Grande", Helvetica, Arial, "Microsoft YaHei";
		-webkit-font-smoothing: antialiased;
		color: #333;
		width: 100%;
	}
	
	.datetime-picker * {
		box-sizing: border-box;
	}
	
	.datetime-picker input {
		width: 100%;
		padding: 5px 10px;
		height: 30px;
		outline: 0 none;
		border: 1px solid #ccc;
		font-size: 13px;
	}
	
	.datetime-picker .picker-wrap {
		/*position: absolute;*/
		z-index: 1000;
		width: 100%;
		/*height: 280px;*/
		margin-top: 2px;
		background-color: #fff;
		box-shadow: 0 0 6px #ccc;
	}
	
	.datetime-picker table {
		width: 100%;
		border-collapse: collapse;
		border-spacing: 0;
		text-align: center;
		font-size: 13px;
	}
	
	.datetime-picker tr {
		height: 34px;
		border: 0 none;
		background-color: #fff;
	}
	
	.datetime-picker th,
	.datetime-picker td {
		user-select: none;
		width: 34px;
		height: 34px;
		padding: 44px 10px 10px;
		border: 0 none;
		line-height: 34px;
		text-align: left;
		position: relative;
	}
	
	.datetime-picker tbody td p{
		position: absolute;
		top: 0;
		left: 10px;
		font-size: 18px;
	}
	
	.datetime-picker td>div {
		text-align: center;
		display: flex;
		justify-content: flex-start;
		align-items: center;
	}
	
	.datetime-picker td>.user{
		margin-bottom: 10px;
	}
	
	.datetime-picker td>.user img {
		width: 80px;
	}
	
	.datetime-picker td>.user span {
		margin-left: 20px;
	}
	
	.datetime-picker tbody th{
		min-height: 120px;
	}
	
	.datetime-picker td {
		cursor: pointer;
		border-bottom: 1px solid #e5e5e5;
	}
	
	.datetime-picker tr>td:not(:last-child) {
		border-right: 1px solid #e5e5e5;
	}
	
	.datetime-picker td:hover {
		background-color: #f0f0f0;
	}
	
	.datetime-picker td.date-pass,
	.datetime-picker td.date-future {
		color: #aaa;
	}
	
	.datetime-picker td.date-active {
		background-color: #ececec;
		color: #3bb4f2;
	}
	
	.datetime-picker .date-head {
		background-color: #409EFF;
		text-align: center;
		color: #fff;
		font-size: 14px;
	}
	
	.datetime-picker .date-days {
		color: #409EFF;
		font-size: 14px;
		border-bottom: solid 1px #e5e5e5;
	}
	
	.datetime-picker .show-year {
		display: inline-block;
		min-width: 62px;
		vertical-align: middle;
	}
	
	.datetime-picker .show-month {
		display: inline-block;
		min-width: 28px;
		vertical-align: middle;
	}
	
	.datetime-picker .btn-prev,
	.datetime-picker .btn-next {
		cursor: pointer;
		display: inline-block;
		padding: 0 10px;
		vertical-align: middle;
	}
	
	.datetime-picker .btn-prev:hover,
	.datetime-picker .btn-next:hover {
		background: rgba(16, 160, 234, 0.5);
	}
</style>

<template>
	<div class="datetime-picker" :style="{ width: width }">
		<div class="picker-wrap">
			<table class="date-picker">
				<thead>
					<tr class="date-head">
						<th colspan="4">
							<span class="btn-prev" @click="yearClick(-1)">&lt;</span>
							<span class="show-year">{{now.getFullYear()}}</span>
							<span class="btn-next" @click="yearClick(1)">&gt;</span>
						</th>
						<th colspan="3">
							<span class="btn-prev" @click="monthClick(-1)">&lt;</span>
							<span class="show-month">{{months[now.getMonth()]}}</span>
							<span class="btn-next" @click="monthClick(1)">&gt;</span>
						</th>
					</tr>
					<tr class="date-days">
						<th v-for="day in days">{{day}}</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="i in 6">
						<td v-for="j in 7" :class="date[i * 7 + j] && date[i * 7 + j].status" :date="date[i * 7 + j] && date[i * 7 + j].date" @click="pickDate(i * 7 + j)">
							<p>{{date[i * 7 + j] && date[i * 7 + j].text}}</p>
							<div class="user" v-if="j == 2 && i == 1">
								上学：
								<img src="../screenshot.png" />
								<span>李爸爸</span>
							</div>
							<div class="user" v-if="j == 2 && i == 1">
								放学：
								<img src="../screenshot.png" />
								<span>李爸爸</span>
							</div>
							<div class="user" v-if="j == 2 && i == 1">
								放学：
								<img src="../screenshot.png" />
								<span>李爸爸</span>
							</div>
							<div class="user">
								放学：
								<img src="../screenshot.png" />
								<span>李爸爸</span>
							</div>
							<div class="user">
								放学：
								<img src="../screenshot.png" />
								<span>李爸爸</span>
							</div>
						</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
</template>

<script>
	export default {
		props: {
			width: {
				type: String,
				default: '100%'
			},
			readonly: {
				type: Boolean,
				default: false
			},
			value: {
				type: String,
				default: ''
			},
			format: {
				type: String,
				default: 'YYYY-MM-DD'
			},
			name: {
				type: String,
				default: ''
			}
		},
		data() {
			return {
				show: true,
				days: ['日', '一', '二', '三', '四', '五', '六'],
				months: ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'],
				date: [],
				now: new Date(),
				pickedValue: ''
			};
		},
		watch: {
			now() {
				this.update();
			},
			show() {
				this.update();
			}
		},
		methods: {
			close() {
				this.show = true;
			},
			update() {
				var arr = [];
				var time = new Date(this.now);
				time.setMonth(time.getMonth(), 1); // the first day
				var curFirstDay = time.getDay();
				curFirstDay === 0 && (curFirstDay = 7);
				time.setDate(0); // the last day
				var lastDayCount = time.getDate();
				for(let i = curFirstDay; i > 0; i--) {
					arr.push({
						text: lastDayCount - i + 1,
						time: new Date(time.getFullYear(), time.getMonth(), lastDayCount - i + 1),
						status: 'date-pass'
					});
				}

				time.setMonth(time.getMonth() + 2, 0); // the last day of this month
				var curDayCount = time.getDate();
				time.setDate(1); // fix bug when month change
				var value = this.pickedValue || this.stringify(new Date());
				for(let i = 0; i < curDayCount; i++) {
					let tmpTime = new Date(time.getFullYear(), time.getMonth(), i + 1);
					let status = '';
					this.stringify(tmpTime) === value && (status = 'date-active');
					arr.push({
						text: i + 1,
						time: tmpTime,
						status: status
					});
				}

				var j = 1;
				while(arr.length < 42) {
					arr.push({
						text: j,
						time: new Date(time.getFullYear(), time.getMonth() + 1, j),
						status: 'date-future'
					});
					j++;
				}
				var userData = [1539532800000];
				arr.forEach(function(item, idx, str){
					userData.forEach(function(item1, idx1, str1){
						console.log(idx);
						console.log(new Date(item.time));
						console.log("item1:",new Date(item1));	
						console.log(item);
						console.log("item111:",item1);
						console.log("item2222:",item.time);
						console.log(new Date(item.time).valueOf());
						console.log((new Date(item1)).getTime());
						console.log((new Date(item.time).valueOf()) === (new Date(item1).valueOf()));
						if((new Date(item.time).getTime()) === (new Date(item1).getTime())) {
							console.log(11111111)
						} else {

						}
					})
				})
				this.date = arr;
			},
			yearClick(flag) {
				this.now.setFullYear(this.now.getFullYear() + flag);
				this.now = new Date(this.now);
			},
			monthClick(flag) {
				this.now.setMonth(this.now.getMonth() + flag, 1);
				this.now = new Date(this.now);
			},
			pickDate(index) {
				this.show = true;
				this.now = new Date(this.date[index].time);
				this.pickedValue = this.stringify();
			},
			parse(str) {
				var time = new Date(str);
				return isNaN(time.getTime()) ? null : time;
			},
			stringify(time = this.now, format = this.format) {
				var year = time.getFullYear();
				var month = time.getMonth() + 1;
				var date = time.getDate();
				var monthName = this.months[time.getMonth()];

				var map = {
					YYYY: year,
					MMM: monthName,
					MM: ('0' + month).slice(-2),
					M: month,
					DD: ('0' + date).slice(-2),
					D: date
				};
				return format.replace(/Y+|M+|D+/g, function(str) {
					return map[str];
				});
			},
			leave(e) {
				if(!this.$el.contains(e.target)) {
					this.close();
				}
			}
		},
		created() {
			this.now = new Date();
		},
		mounted() {
			this.pickedValue = this.value;
			this.update();
			this.$nextTick(() => {
				this.now = this.parse(this.pickedValue) || new Date();
				document.addEventListener('click', this.leave, false);
			});
		},
		beforeDestroy() {
			document.removeEventListener('click', this.leave, false);
		}
	};
</script>